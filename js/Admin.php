...
class Admin extends CI_Controller
{
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'do_update') {
            ...
            $data['mother_name']        = $this->input->post('mother');
            $data['income']             = $this->input->post('income');
            $data['school']             = $this->input->post('school');
            $data['starting_grade']     = $this->input->post('starting');
            $data['class_id']    = $class_id;
            $data['section_id']  = $this->input->post('section_id');
...
public function view_class_attendance_excel($pod = NULL){
        ...
        $max_date = date('Y-m-d', strtotime('Sunday this week',strtotime($max_date)));
		...
		foreach($students as $student){
          ...
          foreach($weeks as $i=>$week){
            ...
            if($i!='0'){
                ...
                $html .= '<td>'.$present.' / '.count($attendance).'<p>'.$stats.'</p></td>';
            }
            $week_s = $weeks[$i];
          }
          $html.= '</tr>'; 
